...
const char* DirectorySystem::PrintError(int error_code)
{
	...
	case ESRCH: return "No such process || Insufficient Info for file access";
...
bool DirectorySystem::SeekFile(HANDLE file, int64 offset, FilePosMode mode)
{
	...
	switch (mode)
	{
	case FilePosMode::BEGIN: method = FILE_BEGIN; break;
	case FilePosMode::CURRENT: method = FILE_CURRENT; break;
	case FilePosMode::END: method = FILE_END; break;
	}
...
HANDLE DirectorySystem::CreateFiile(const char* dest)
{
	...
	if (newFileHandle == INVALID_HANDLE)
	{
		...
		THROW("lv_file_create Code = %i : %s (%s)", error_code, DirectorySystem::PrintError(error_code), dest);
	}

	return newFileHandle;
}

HANDLE DirectorySystem::OpenFile(const char* path, FileAccessMode accessFlag, FileOpenMode modeFlag)
{
	char conv[INIT_STR_LENGTH] = { 0 };
...
int64 DirectorySystem::ReadFromFile(HANDLE file, void* buffer, uint64 nbytes)
{
	...
	return ReadFile(file, buffer, static_cast<DWORD>(nbytes), &readBytes, nullptr) ? static_cast<int64>(readBytes) : -1;

}

int64 DirectorySystem::WriteToFile(HANDLE file, const void* buffer, uint64 nbytes)
{
	DWORD writeBytes = 0;

	// buffer 포인터 시작점으로부터 nbytes 만큼의 데이터를 실제 file 에 쓰는 것이다.
	// 리턴값은 실제 파일에 쓴 바이트 숫자이다.
	return WriteFile(file, buffer, static_cast<DWORD>(nbytes), &writeBytes, nullptr) ? static_cast<int64>(writeBytes) : -1;
...
std::string DirectorySystem::CombinePath(const char* a, const char* b)
{
	...
	int32 newCapacity = aLen + bLen + 2; // 1 : DIRECTORY_SEPARATOR_CHAR, 2 : null limiter
...
void DirectorySystem::FromSystemToUTF8Path(char* dest_utf8, const void* src_sys)
{
	...
	*/
	const int size = WideCharToMultiByte(CP_UTF8, WC_NO_BEST_FIT_CHARS, buf, -1, NULL, NULL, NULL, NULL);
...
void DirectorySystem::FromUTF8ToSystemPath(void* dest_sys, const char* src_utf8, bool need_safe_conversion)
{
	char src[INIT_STR_LENGTH] = { 0, };
	...
	if (size > 0)
	{
		...
		MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, src, -1, dest, size + Utils::StringUtil::CalculateUTF8ExtraLength(src_utf8));
	}
	...
	if (need_safe_conversion)
	{
		...
		{
			WCHAR w = dest[i];
			if (w == 0xFFFD)	// 0xFFFD represents a Unicode replacement character (�) indicating an invalid character
			{
				isANSI = true;
				break;
			}
		...
		if (isANSI)
		{
			...
			if (size > 0)
			{
				...
				MultiByteToWideChar(CP_ACP, MB_ERR_INVALID_CHARS, src, -1, dest, size + Utils::StringUtil::CalculateUTF8ExtraLength(src_utf8));
			}
		}
	}
}
