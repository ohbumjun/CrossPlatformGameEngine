                                    ...
                                    uint32_t Offset)
{
    ...
    return RegisterField(GetTypeID<Class>(),
...
FieldInfo Reflection::RegisterField(TypeId classId,
...
const FieldInfo *Reflection::GetFieldInfo(TypeId classId,
                                          const std::string &FieldName)
{
    static StaticContainerData &containerData = getStaticContainerData();
...
bool Reflection::IsDerivedClass(TypeId baseType, TypeId childType)
{
    ...
    if (!IsRegistered(baseType))
        THROW("Type is not registed");
    if (!IsRegistered(childType))
        THROW("Type is not registed");
    if (dinfo == nullptr)
        THROW("Type is not registed");
    ...
    bool foundResult = std::find(baseInfo->m_SubTypes.begin(),
                                 baseInfo->m_SubTypes.end(),
                                 childType) != baseInfo->m_SubTypes.end();
...
void Reflection::RegisterBase(TypeId baseType, TypeId subType)
{
    ...
    bool contain = std::find(baseTypeInfo->m_SubTypes.begin(),
                             baseTypeInfo->m_SubTypes.end(),
                             subType) != baseTypeInfo->m_SubTypes.end();
