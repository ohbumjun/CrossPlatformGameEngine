...
JsonSerializer::JsonSerializer() : Serializer(SerializeType::Serialize)
{
    ...
    rapidjson::StringBuffer *sbWriter =
        (rapidjson::StringBuffer *)m_StringBuffer;

    m_JsonWriter = new JsonWriter(*sbWriter);

    static_cast<JsonWriter *>(m_JsonWriter)
        ->SetFormatOptions(rapidjson::kFormatSingleLineArray);
}
...
std::string JsonSerializer::GetFinalResult()
{
    rapidjson::StringBuffer *StringBuffer =
        static_cast<rapidjson::StringBuffer *>(m_StringBuffer);
...
void JsonSerializer::onLoad(glm::vec2 &data)
{
    ...
    rapidjson::Value *arrayValue =
        static_cast<rapidjson::Value *>(getValue(prevRecord));
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        val = &prevValue->operator[](
            static_cast<SizeType>(prevArrayRecord.m_ArrayElemNum++));
...
void JsonSerializer::onLoadBuffer(void *buffer, size_t size)
{
    ...
    // string -> buffer
    Utils::Encoder::base64_decode(const_cast<char *>(encodeData.c_str()),
                                  encodeSize,
                                  dest);
}
...
void *JsonSerializer::getValue(JsonRecord &prevRecord)
{
	...
	*/
    ...
    if (nullptr != prevValue && prevValue->IsArray())
    {
        ...
        // Array 상에서 특정 index 에 있는 값을 가져온다.
        v = &prevValue->operator[](
            static_cast<SizeType>(prevRecord.m_ArrayElemNum++));
    }
    ...
    else if (nullptr != prevRecord.value && prevValue->IsObject())
    {
        ...
        {
            v = &prevValue->operator[](
                Value(key, static_cast<SizeType>(strlen(key))));
        }
    }
