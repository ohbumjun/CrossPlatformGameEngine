...
void SceneHierarchyPanel::OnImGuiRender()
{
...

    {
        ...
        if (ImGui::BeginPopupContextWindow(0,
                                           1 | ImGuiPopupFlags_NoOpenOverItems))
        {
...
void SceneHierarchyPanel::drawEntityNode(Entity entity)
{
    ...
    bool isOpened = ImGui::TreeNodeEx((void *)(uint64_t)(uint32_t)entity,
                                      flags,
                                      name.c_str());
    ...
    {
        {
            ...
            ImGuiTreeNodeFlags flags =
                ...
                ImGuiTreeNodeFlags_OpenOnArrow;
            bool isOpened =
                ImGui::TreeNodeEx((void *)(uint64_t)(uint32_t)entity,
                                  flags,
                                  name.c_str());
            flags |= ImGuiTreeNodeFlags_SpanAvailWidth;
            if (isOpened)
...
{
    HazelEditor::DrawComponent<TransformComponent>(
        "Transform",
        entity,
        [](auto &component) {
            HazelEditor::DrawVec3Control("Translation",
                                         component.GetTranslationRef());
...
{
    HazelEditor::DrawComponent<CameraComponent>(
        "Camera",
        entity,
        &HazelEditor::CameraPanel::DrawCameraComponent);
}

void SceneHierarchyPanel::drawRendererComponent(Entity entity)
{

    HazelEditor::DrawComponent<CircleRendererComponent>(
        "Circle Renderer",
        entity,
        [](auto &component) {
            ImGui::ColorEdit4("Color", glm::value_ptr(component.GetColorRef()));
            ImGui::DragFloat("Thickness",
                             &component.GetThickNessRef(),
                             0.025f,
                             0.0f,
                             1.0f);
            ImGui::DragFloat("Fade",
                             &component.GetFadeRef(),
                             0.00025f,
                             0.0f,
                             1.0f);
        });
    ...
    HazelEditor::DrawComponent<SpriteRenderComponent>(
        "Sprite",
        entity,
        [](auto &component) {
            ImGui::ColorEdit4("Color", glm::value_ptr(component.GetColorRef()));
            ...
            if (ImGui::BeginDragDropTarget())
            {
                        ...
                        ImGui::AcceptDragDropPayload("CONTENT_BROWSER_ITEM"))
                {
                    ...
                    std::filesystem::path texturePath =
                        std::filesystem::path(g_AssetPath) / path;

                    if (Utils::isSupportedImageFormat(texturePath.string()))
                    {
                        Ref<Texture2D> texture =
                            TextureManager::CreateTexture2D(
                                texturePath.string());
                        ...
                        {
                            HZ_WARN("Could not load texture {0}",
                                    texturePath.filename().string());
                        }
                    }
                    else
                    {
                        HZ_WARN("Invalid Texture Format {0}",
                                texturePath.filename().string());
                    }
                }
            ...
            // Image
            ImGui::Image(component.GetTexture()
                             ? (void *)component.GetTexture()->GetRendererID()
                             : nullptr,
                         {imageSize, imageSize},
                         {0, 1},
                         {1, 0});

            ImGui::DragFloat("Tiling Factor",
                             &component.GetTilingFactorRef(),
                             0.1f,
                             0.0f,
                             100.0f);
        });
...
void SceneHierarchyPanel::drawColliderComponent(Entity entity)
{
    ...
    HazelEditor::DrawComponent<BoxCollider2DComponent>(
        "Box Collider 2D",
        entity,
        [](auto &component) {
            ImGui::DragFloat2("Offset",
                              glm::value_ptr(component.GetOffsetRef()));
            ImGui::DragFloat2("Size", glm::value_ptr(component.GetSizeRef()));
            ImGui::DragFloat("Density",
                             &component.GetDensityRef(),
                             0.01f,
                             0.0f,
                             1.0f);
            ImGui::DragFloat("Friction",
                             &component.GetFrictionRef(),
                             0.01f,
                             0.0f,
                             1.0f);
            ImGui::DragFloat("Restitution",
                             &component.GetRestitutionRef(),
                             0.01f,
                             0.0f,
                             1.0f);
            ImGui::DragFloat("Restitution Threshold",
                             &component.GetRestitutionThresholdRef(),
                             0.01f,
                             0.0f);
        });

    HazelEditor::DrawComponent<CircleCollider2DComponent>(
        "Circle Collider 2D",
        entity,
        [](auto &component) {
            ImGui::DragFloat2("Offset",
...
{
    HazelEditor::DrawComponent<Rigidbody2DComponent>(
        "Rigidbody 2D",
        entity,
        [](auto &component) {
            const char *bodyTypeStrings[] = {"Static", "Dynamic", "Kinematic"};
            const char *currentBodyTypeString =
                bodyTypeStrings[(int)component.GetBodyType()];
            if (ImGui::BeginCombo("Body Type", currentBodyTypeString))
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    if (ImGui::Selectable(bodyTypeStrings[i], isSelected))
                    {
                        ...
                        component.SetBodyType(
                            (Rigidbody2DComponent::BodyType)i);
                    }
...
void SceneHierarchyPanel::drawAddComponentPanel()
{
    ...
    if (ImGui::BeginPopup("AddComponent"))
    {
        ...
        displayAddComponentEntry<CircleCollider2DComponent>(
            "Circle Collider 2D");
        ImGui::EndPopup();
    }
}
